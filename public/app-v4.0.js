...
const app = {
    ...
    // V4.0 - Nuevas propiedades
    compareList: [], // Lista de productos a comparar
    conversationHistory: [], // Historial completo de conversaciones
    preferences: null, // Preferencias del usuario
    notifications: [], // Notificaciones
    theme: 'light', // Tema actual
    lastSearchResults: [], // Resultados de búsqueda para paginación
    lastCategoryResults: [] // Resultados de categoría para paginación
};
...
// ==================== V4.0 - CONSTANTES Y CONFIGURACIÓN ====================
...
const V4_CONFIG = {
    ...
    QUICK_COMMANDS: [
        { command: '/ofertas', description: 'Ver productos en oferta', icon: '🏷️' },
        { command: '/destacados', description: 'Ver productos destacados', icon: '⭐' },
        { command: '/cupones', description: 'Ver cupones disponibles', icon: '🎟️' },
        { command: '/comparar', description: 'Comparar productos seleccionados', icon: '⚖️' },
        { command: '/ayuda', description: 'Ver lista de comandos', icon: '❓' },
        { command: '/limpiar', description: 'Limpiar chat', icon: '🗑️' }
    ],
    TYPING_SPEED: 30, // ms por carácter
    MESSAGE_DELAY: 500 // ms antes de mostrar mensaje
};
...
function createPartCardV3(part) {
    ...
    // V4.1 - Diseño más compacto
    // Botón de gestión de imágenes (solo para admin)
    const imageManagementBtn = (auth.user && auth.user.is_admin) ?
        `<button class="btn-icon-compact" onclick="showImageManager(${partId})" title="Gestionar imágenes">🖼️</button>` : '';

    return `
        <div class="part-card-v4-1 animate-in" data-part-id="${partId}">
            <img src="${imageUrl}" alt="${part.name}" class="part-image-compact" onerror="this.src='https://via.placeholder.com/300x300/3498db/ffffff?text=Repuesto'" onclick="viewProductImages(${partId})">

            <div class="part-info-compact">
                <div class="part-header-compact">
                    <div class="part-name-compact">${part.name}</div>
                    <span class="brand-badge-compact" style="background: ${brandColor};">🏍️ ${brandName}</span>
                </div>

                <div class="part-meta-compact">
                    ${categoryName ? `<span class="category-tag">📁 ${categoryName}</span>` : ''}
                    <span class="rating-compact">${stars} ${reviewCount > 0 ? `(${reviewCount})` : ''}</span>
                </div>

                <div class="part-price-compact">$${formatPrice(part.price)}</div>

                <div class="part-footer-compact">
                    <span class="stock-compact ${part.stock > 10 ? 'in-stock' : 'low-stock'}">📦 ${part.stock}</span>
                    <div class="actions-compact">
                        <button class="btn-icon-compact btn-cart-compact" onclick="addToCart(${partId})" title="Carrito">🛒</button>
                        <button class="btn-icon-compact btn-fav-compact" onclick="addToFavorites(${partId})" title="Favoritos">❤️</button>
                        <button class="btn-icon-compact btn-compare-compact" onclick="addToCompare(${partId})" title="Comparar">⚖️</button>
                        <button class="btn-icon-compact btn-review-compact" onclick="showReviews(${partId})" title="Opiniones">⭐</button>
                        ${imageManagementBtn}
                    </div>
                </div>
            </div>
...
async function processMessage(message) {
    ...
    // V4.0 - Detectar comandos rápidos
    if (lowerMessage.startsWith('/')) {
        const result = await executeCommand(lowerMessage);
        if (result) return result;
        return null; // El comando manejó su propia visualización
    }
    ...
    if (lowerMessage.includes('mostrar todo') || lowerMessage.includes('catalogo') || lowerMessage.includes('catálogo')) {
        ...
        // Mostrar solo 6 productos inicialmente
        const initialShow = 6;
        app.parts.slice(0, initialShow).forEach(part => {
            html += createPartCardV3(part);
        });

        // Botón para ver más si hay más productos
        if (app.parts.length > initialShow) {
            html += `
                <div style="text-align: center; margin: 20px 0;">
                    <button onclick="showMoreProducts('catalog', ${initialShow})" class="btn-primary">
                        Ver más productos (${app.parts.length - initialShow} restantes)
                    </button>
                </div>
            `;
        }
...
async function handleSearch(message) {
    ...
    if (categoryMatch) {
        ...
        if (categoryParts.length > 0) {
            ...
            app.lastCategoryResults = categoryParts;
            ...
            // Mostrar solo 6 productos inicialmente
            const initialShow = 6;
            categoryParts.slice(0, initialShow).forEach(part => {
                html += createPartCardV3(part);
            });

            // Botón para ver más si hay más productos
            if (categoryParts.length > initialShow) {
                html += `
                    <div style="text-align: center; margin: 20px 0;">
                        <button onclick="showMoreProducts('category', ${initialShow})" class="btn-primary">
                            Ver más productos (${categoryParts.length - initialShow} restantes)
                        </button>
                    </div>
                `;
            }
    ...
    try {
        ...
        if (parts.length > 0) {
            ...
            // Mostrar solo 6 productos inicialmente
            const initialShow = 6;
            parts.slice(0, initialShow).forEach(part => {
                html += createPartCardV3(part);
            });

            // Botón para ver más si hay más productos
            if (parts.length > initialShow) {
                html += `
                    <div style="text-align: center; margin: 20px 0;">
                        <button onclick="showMoreProducts('search', ${initialShow})" class="btn-primary">
                            Ver más resultados (${parts.length - initialShow} restantes)
                        </button>
                    </div>
                `;
            }
            ...
            if (isPriceQuery) {
                const total = parts.slice(0, initialShow).reduce((sum, p) => sum + p.price, 0);
...
🏍️ ChatBot de Repuestos para Motos Colombianas - Versión 4.0
${gptStatus}

💬 Búsqueda y navegación:
• Escribe el nombre de un repuesto (con autocompletado)
...
• /comparar - Comparar productos seleccionados
• /limpiar - Limpiar chat

✨ Funcionalidades v4.0:
• 🔍 Autocompletado inteligente al escribir
• ⚖️ Comparador de productos (hasta 3)
• 🎟️ Sistema de cupones y descuentos
• 🌙 Modo oscuro/claro (botón arriba)
• 💡 Productos relacionados y recomendaciones
• 📸 Imágenes y calificaciones de productos

🛒 Acciones en productos:
• 🛒 Agregar al carrito
• ❤️ Agregar a favoritos
• ⭐ Ver opiniones y calificar
• ⚖️ Agregar a comparación (nuevo)
${!auth.user ? '• Inicia sesión para comprar, calificar y comparar' : '• ¡Ya tienes acceso completo!'}

${!app.useGPT ? '💡 Tip: Para activar IA GPT, agrega créditos en platform.openai.com' : ''}

¿Qué necesitas hoy?`;

        addMessage(welcomeMsg, false);
        console.log('Aplicación v4.0 iniciada correctamente');
        console.log('GPT activado:', app.useGPT);

        // V4.0 - Inicializar módulos adicionales
        setupAutocomplete();
        initTheme();
        updateCompareUI();
        checkOnboarding();
